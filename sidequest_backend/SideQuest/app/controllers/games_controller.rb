class GamesController < ApplicationController
    ...
    def show
        ...
        render json: {
            id: game.id, dungeon_master: dungeon_master_user, rplevel: game.rplevel, name: game.name, explevel: game.explevel, party: partyInfo, players: game.party_members.map{ | member | member.character.user}, location: game.location, desc: game.desc, messages: game.messages.map{ | message| {id: message.id, user: message.user, game: message.game, content: message.content}}
        }
    end
    ...
    def game_params
        params.require(:game).permit(:name, :rplevel, :explevel, :location, :desc)
    end
end
